...
public class TournamentSteps {
  ...
  private void enrollPlayer(String name) {
    TournamentPlayerEnrolled event = TournamentPlayerEnrolled.newBuilder()
  ...
  private void createDefaultTournament(String name, int maxPlayers) {
    TournamentCreated event = TournamentCreated.newBuilder()
        .setName(name)
  ...
  public void registrationClosed() {
    state.applyRegistrationClosed(RegistrationClosed.newBuilder()
  ...
  public void tournamentStarted() {
    state.applyTournamentStarted(TournamentStarted.newBuilder()
  ...
  public void runningTournamentWithRebuys(int maxRebuys, int cutoffLevel) {
    ...
    for (int i = 0; i < 10; i++) {
      blinds.add(BlindLevel.newBuilder()
          .setLevel(i + 1).setSmallBlind((i + 1) * 25L).setBigBlind((i + 1) * 50L).build());
    }
    TournamentCreated event = TournamentCreated.newBuilder()
        .setName("Rebuy Tournament")
        .setGameVariant(GameVariant.TEXAS_HOLDEM)
        .setBuyIn(1000).setStartingStack(10000).setMaxPlayers(100).setMinPlayers(2)
        .setRebuyConfig(RebuyConfig.newBuilder()
            .setEnabled(true).setMaxRebuys(maxRebuys).setRebuyLevelCutoff(cutoffLevel)
            .setRebuyCost(1000).setRebuyChips(10000).build())
  ...
  public void runningTournamentWithBlindStructure(int levels) {
    ...
    TournamentCreated event = TournamentCreated.newBuilder()
        .setName("Blind Tournament").setGameVariant(GameVariant.TEXAS_HOLDEM)
        .setBuyIn(1000).setStartingStack(10000).setMaxPlayers(100).setMinPlayers(2)
        .addAllBlindStructure(blinds).build();
    state.applyCreated(event);
    registrationOpened();
    nPlayersEnrolled(3);
    registrationClosed();
  ...
  public void currentBlindLevel(int level) {
    for (int i = 0; i < level; i++) {
      state.applyBlindAdvanced(BlindLevelAdvanced.newBuilder()
          .setLevel(i + 1).setSmallBlind((i + 1) * 25L).setBigBlind((i + 1) * 50L).build());
    }
  }
  ...
  public void playerAtPositionEliminated(int position) {
    ...
    state.applyPlayerEliminated(PlayerEliminated.newBuilder()
  ...
  @When("I handle a CreateTournament command with name {string} buy-in {int} and starting-stack {int}")
  public void handleCreateTournament(String name, int buyIn, int startingStack) {
    CreateTournament cmd = CreateTournament.newBuilder()
        .setName(name).setGameVariant(GameVariant.TEXAS_HOLDEM)
        .setBuyIn(buyIn).setStartingStack(startingStack)
        .setMaxPlayers(100).setMinPlayers(2).build();
    handleCommand(() -> CreateHandler.handle(cmd, state));
  }

  @When("I handle a CreateTournament command with name {string} buy-in {int} starting-stack {int} and max-players {int}")
  public void handleCreateTournamentMaxPlayers(String name, int buyIn, int startingStack, int maxPlayers) {
    CreateTournament cmd = CreateTournament.newBuilder()
        .setName(name).setGameVariant(GameVariant.TEXAS_HOLDEM)
        .setBuyIn(buyIn).setStartingStack(startingStack)
        .setMaxPlayers(maxPlayers).setMinPlayers(2).build();
    handleCommand(() -> CreateHandler.handle(cmd, state));
  }

  @When("I handle an OpenRegistration command")
  public void handleOpenRegistration() {
    handleCommand(() -> RegistrationHandler.handleOpen(OpenRegistration.getDefaultInstance(), state));
  }

  @When("I handle a CloseRegistration command")
  public void handleCloseRegistration() {
    handleCommand(() -> RegistrationHandler.handleClose(CloseRegistration.getDefaultInstance(), state));
  }
  ...
  public void handleAdvanceBlindLevel() {
    handleCommand(() -> LifecycleHandler.handleAdvanceBlind(AdvanceBlindLevel.getDefaultInstance(), state));
  }
  ...
  public void handleResumeTournament() {
    handleCommand(() -> LifecycleHandler.handleResume(ResumeTournament.getDefaultInstance(), state));
  }
  ...
  public void eventHasRegistrationNumber(int expected) {
    assertThat(resultEvent).isInstanceOf(TournamentPlayerEnrolled.class);
    assertThat(((TournamentPlayerEnrolled) resultEvent).getRegistrationNumber()).isEqualTo(expected);
  }
  ...
  public void stateHasStatus(String expected) {
    TournamentStatus expectedStatus = switch (expected) {
