...
public class HandSteps {
  ...
  public void cardsDealtEventWithPlayers(int handNumber, int playerCount) {
    ...
    for (int i = 0; i < playerCount; i++) {
      ...
      builder.addPlayers(
          PlayerInHand.newBuilder().setPlayerRoot(playerRoot).setPosition(i).setStack(500).build());

      // Add hole cards for this player
      PlayerHoleCards.Builder holeCardsBuilder =
          PlayerHoleCards.newBuilder().setPlayerRoot(playerRoot);
      for (int j = 0; j < 2; j++) {
  ...
  public void cardsDealtEventForVariantWithStacks(String variant, int playerCount, int stack) {
    ...
    CardsDealt.Builder builder =
        CardsDealt.newBuilder().setHandNumber(1).setGameVariant(gameVariant).setDealerPosition(0);
  ...
  public void cardsDealtEventForVariantWithPlayersTable(
      String variant, io.cucumber.datatable.DataTable dataTable) {
    GameVariant gameVariant = GameVariant.valueOf(variant);
  ...
  public void bettingRoundCompleteEvent(String phase) {
    ...
    BettingRoundComplete event =
        BettingRoundComplete.newBuilder().setCompletedPhase(bettingPhase).build();
    addEvent(event);
    rehydrateHand();
  }
  ...
  public void communityCardsDealtEvent(String phase) {
    ...
    CommunityCardsDealt event =
        CommunityCardsDealt.newBuilder().setPhase(bettingPhase).addAllCards(newCards).build();
    addEvent(event);
    rehydrateHand();
  }
  ...
  public void cardsRevealedEventForPlayer(String playerId, String ranking) {
    HandRanking handRanking =
        HandRanking.newBuilder().setRankType(HandRankType.valueOf(ranking)).build();
    CardsRevealed event =
  ...
  public void handAtShowdownWithCards(
      String playerId, String holeCardsStr, String communityCardsStr) {
    // Parse hole cards
    ...
    CardsDealt cardsDealt =
        CardsDealt.newBuilder()
            ...
            .setDealerPosition(0)
            .addPlayers(
                PlayerInHand.newBuilder().setPlayerRoot(playerRoot).setPosition(0).setStack(500))
            .addPlayers(
                PlayerInHand.newBuilder().setPlayerRoot(player2Root).setPosition(1).setStack(500))
            .addPlayerCards(
                PlayerHoleCards.newBuilder().setPlayerRoot(playerRoot).addAllCards(holeCards))
            .addPlayerCards(
  ...
  public void handleDealCardsCommandForVariant(
      String variant, io.cucumber.datatable.DataTable dataTable) {
    GameVariant gameVariant = GameVariant.valueOf(variant);
    List<java.util.Map<String, String>> players = dataTable.asMaps();

    DealCards.Builder builder =
        DealCards.newBuilder().setHandNumber(1).setGameVariant(gameVariant).setDealerPosition(0);
  ...
  public void handleDealCardsCommandWithSeed(
      String seed, io.cucumber.datatable.DataTable dataTable) {
    List<java.util.Map<String, String>> players = dataTable.asMaps();
